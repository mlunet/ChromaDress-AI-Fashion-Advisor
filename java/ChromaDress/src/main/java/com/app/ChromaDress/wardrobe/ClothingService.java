...
public class ClothingService {
    ...
    public List<ClothingResponseDTO> getUserCloset() {
        ...
        return clothingRepository.findByUserId(user.getId())
                .stream()
    ...
    public List<OutfitSuggestionDTO> getOutfitSuggestions(Long clothingId, PaletteType type) {
        ...
        List<List<String>> allPalettes = analysisFacade.getSuggestedPalette(selectedItem.getHexColor(), type);
        ...
        for (int i = 0; i < allPalettes.size(); i++) {
            ...
            for (String hexSuggestion : palette) {
                ...
                List<ClothingItem> matches = clothingRepository.findSimilarHSL(
                        h, s, l, clothingId, selectedItem.getCategory()
                );
    ...
    @Transactional
    @Caching(evict = {@CacheEvict(value = "wadaPalettes", key = "#result"), @CacheEvict(value = "classicPalettes", key = "#result")})
